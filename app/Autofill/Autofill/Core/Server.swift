...
final class Server: ObservableObject {
    ...
    func start(demo: StoreData? = nil) {
        ...
        do {
            ...
            l.stateUpdateHandler = { [weak self] state in
                Task { @MainActor in
                    ...
                    case .failed(let e):
                        self?.running = false
                        self?.lastError = "\(e)"
                        NSLog("Autofill: listener failed: \(e)")
                    case .waiting(let e):
                        self?.lastError = "waiting: \(e)"
                        NSLog("Autofill: listener waiting: \(e)")
            ...
            lastError = "\(error)"
            running = false
            NSLog("Autofill: listener could not start: \(error)")
        }
    }
        ...
        if path == "/test" || path == "/test/" {
            let url = Bundle.main.url(forResource: "testpage", withExtension: "html")
                ?? Bundle.main.builtInPlugInsURL?
                    .appendingPathComponent("Autofill Extension.appex/Contents/Resources/testpage.html")
    ...
    static let logURL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Logs/Autofill.log")
