...
@main
struct AutofillApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate

    var body: some Scene {
        Window("Autofill", id: "settings") {
            SettingsView(model: AppModel.shared)
                // `open autofill://settings` opens this window from anywhere,
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc private func clicked() {
        if NSApp.currentEvent?.type == .rightMouseUp {
            let menu = NSMenu()
            menu.addItem(withTitle: "Open Autofill", action: #selector(openSettings), keyEquivalent: "")
                .target = self
            menu.addItem(.separator())
            menu.addItem(withTitle: "Quit Autofill",
    ...
    @objc private func openSettings() {
        ...
        if let w = NSApp.windows.first(where: { $0.title == "Autofill" }) {
            w.makeKeyAndOrderFront(nil)
        } else {
            NSWorkspace.shared.open(URL(string: "autofill://settings")!)
        }
    }
}
...
final class AppModel: ObservableObject {
    ...
    init() {
        NSLog("Autofill: model starting, demo=\(demo)")
        ...
        if let i = CommandLine.arguments.firstIndex(of: "-ImportFile"),
           ...
           let incoming = try? JSONDecoder().decode(StoreData.self, from: blob) {
            ...
            NSLog("Autofill: imported \(incoming.people.count) people at start up")
        }
        ...
        if let i = CommandLine.arguments.firstIndex(of: "-ExportFile"),
           i + 1 < CommandLine.arguments.count {
            ...
            NSLog("Autofill: exported \(data.people.count) people at start up")
        }
    ...
    func export() {
        ...
        panel.nameFieldStringValue = "autofill-export.json"
    ...
    func importFile() {
        ...
        do {
            ...
            note = "That file does not look like an Autofill export: \(error.localizedDescription)"
        }
    }
